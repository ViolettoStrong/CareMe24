...
class PaySwitcherNotifMe extends StatefulWidget {
  final bool? isVisible;
  final bool on;
  final Function(bool)? onChanged;

  const PaySwitcherNotifMe({
    this.isVisible = true,
    this.onChanged,
    this.on = false,
    super.key,
  ...
  @override
  State<PaySwitcherNotifMe> createState() => _PaySwitcherNotifMe();
}

class _PaySwitcherNotifMe extends State<PaySwitcherNotifMe>
  ...
  Future<void> _loadSavedSwitchValue() async {
    ...
    final saved = prefs.getBool('pay_switch_value_notif_me');
    if (saved != null) {
      _controllerOn.value = saved;
      _updateStyleBasedOnState(saved);
      if (saved) {
  ...
  void _initAnimation() {
    // const offset = 40.0; // Half of the switch width (80 / 2)
    // _slideAnimation = Tween<Offset>(
    //   begin: const Offset(-offset, 0),
    //   end: const Offset(offset, 0),
  ...
  void _onSwitchChanged() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('pay_switch_value_notif_me', newValue);
...
class PaySwitcherNotiftoMe extends StatefulWidget {
  final bool? isVisible;
  final bool on;
  final Function(bool)? onChanged;

  const PaySwitcherNotiftoMe({
    this.isVisible = true,
    this.onChanged,
    this.on = false,
    super.key,
  ...
  @override
  State<PaySwitcherNotiftoMe> createState() => _PaySwitcherNotiftoMe();
}

class _PaySwitcherNotiftoMe extends State<PaySwitcherNotiftoMe>
    with TickerProviderStateMixin {
  late ValueNotifier<bool> _controllerOn;
  late AnimationController _animationController;
  // late Animation<Offset> _slideAnimation;
  ...
  Future<void> _loadSavedSwitchValue() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = prefs.getBool('pay_switch_value_notif_tome');
    if (saved != null) {
      _controllerOn.value = saved;
      _updateStyleBasedOnState(saved);
      if (saved) {
  ...
  void _onSwitchChanged() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('pay_switch_value_notif_tome', newValue);
