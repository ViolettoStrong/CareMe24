...
class RescueCallButton extends StatefulWidget {
  const RescueCallButton({
    super.key,
    required this.text,
  ...
  @override
  State<RescueCallButton> createState() => _RescueCallButtonState();
}

class _RescueCallButtonState extends State<RescueCallButton> {
  bool on = false;
  ...
  Future<void> _loadNearestInstitution() async {
    ...
    final res = await Api.loadNearestInstitution(
        lat: userLat, lon: userLon, institutionType: 'mch');
    if (res != null) {
                        ...
                        if (on) {
                          ...
                          if (resopnse.isSuccess) {
                            if (widget.selectedContact != null) {
                              ...
                              if (cardId != null) {
                                dynamic res =
                                    await Api.fetchCallsData('mch', cardId.id);
                                if (res != null) {
                                  ElegantNotification.success(
                                          description: const Text(
                                              'Вызов успешно отправлен'))
                            ...
                            ElegantNotification.error(
                                    description:
                                        const Text('Неудалось вызвать MЧС'))
                                .show(context);
                            setState(() {
                              on = false;
                            });
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    const SelectInstituts(type: 'mch')))
                        .then((result) async {
