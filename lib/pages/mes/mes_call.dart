...
class CallMESScreen extends StatefulWidget {
  ...
  const CallMESScreen(this.appbartitle, this.type, {super.key});
...
class _CallMESScreenState extends State<CallMESScreen> {
  ...
  final TextEditingController _controllerestimationstart =
      TextEditingController();
  final TextEditingController _controllerestimationend =
      TextEditingController();
  ...
  final TextEditingController _controllerdistancestart =
      TextEditingController();
  ...
  Widget build(BuildContext context) {
    AppBloc.serviceCubit.fetchData(widget.type);
    return SafeArea(
      child: Scaffold(
        key: _scaffoldKey,
            ...
            actions: [
              Builder(builder: (context) {
                return AppbarImage(
                    ...
                    margin: getMargin(left: 26, top: 9, right: 26, bottom: 17));
              })
            ],
            styleType: Style.bgFillBlue60001),
        body: BlocBuilder<ServiceCubit, ServiceState>(
          builder: (context, _) {
            ServiceLoaded? state = context.read<ServiceCubit>().loadedState;
            if (state != null) {
              return SizedBox(
                width: size.width,
                      ...
                      children: [
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => SelectReasonScreen(
                                    type: widget.type == 'pol'
                                        ? 'pol'
                                        : widget.type == 'mch'
                                            ? 'mes'
                                            : widget.type == 'med'
                                                ? 'med'
                                                : '', // Default value to handle unexpected cases
                                  ),
                                ),
                              ).then((reason) {
                                setState(() {
                            ...
                            child: Padding(
                              ...
                              child: Container(
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(10),
                                  color:
                                      const Color.fromRGBO(178, 218, 255, 100),
                                ),
                                width: MediaQuery.of(context).size.width - 40,
                                height: 80,
                                child: Padding(
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              if (widget.type == 'mch')
                                ...List.generate(state.serviceList.length,
                                    (index) {
                                  final item = state.serviceList[index];

                                  final Color bgColor = item.price == 0
                                      ? Colors.white
                                      : VersionConstant.free
                                          ? Colors.pink.shade100
                                          : Colors.grey.shade300;
                                  final bool canWork = VersionConstant.free
                                      ? item.price > 0
                                      : item.price == 0;

                                  return MESCard(
                                    medCardId: state.medCardId,
                                    mes_image: item.photo,
                                ...
                                ...List.generate(state.serviceList.length,
                                  ...
                                  return DoctorCard(
                                    ...
                                    verified: state.serviceList[index].verified,
                                    reason: selectedReason ?? '',
                                    serviceModel: item,
                                    doctor_image:
                                        state.serviceList[index].photo,
