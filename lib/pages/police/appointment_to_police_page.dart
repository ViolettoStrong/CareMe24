...
class AppointmentToPoliceScreen extends StatefulWidget {
  ...
  final String cardId;
  const AppointmentToPoliceScreen(
      {required this.serviceModel, required this.cardId, super.key});
...
class _AppointmentToPoliceScreenState extends State<AppointmentToPoliceScreen> {
  ...
  callBackTime(String? varTime) {
    setState(() {
      selectedTime = varTime;
      print(selectedTime);
    });
  }

  bool isRecordSelected() {
    return selectedTime != null && isSelectedIndex != -1;
  }
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    if (isRecordSelected()) {
                      final now = DateTime.now();
                      final selectedDate =
                          now.add(Duration(days: isSelectedIndex));
                      ...
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context) => PaymentDefoultScreen(
                                    ...
                                    summa: widget.serviceModel.price.toInt(),
                                    cardId: widget.cardId,
                                    appointmentTime: appointmentTime.toUtc(),
                                    type: 'pol',
                                  )));
                    }
                  },
